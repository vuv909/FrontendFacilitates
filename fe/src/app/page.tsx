...
export default function Home() {
  ...
  const handleSendMessage = () => {
    ...
    if (user) {
      chat({ userId: user?._id, type: "user", content: inputText })
        .then(response => {
          socket?.emit('privateMessage', { sender: user?._id, message: inputText })
        })
        .catch(error => {
  ...
  return (
    <div>
      ...
      <div className="fixed bottom-5 right-5 bg-white p-4 border border-gray-300 overflow-y-auto" style={{ maxHeight: '400px' }}>
  <h2 className="text-lg font-semibold mb-2">Chat Box</h2>
  <div className="flex flex-col space-y-2">
    {messages.map((message, index) => (
      <div
        key={index}
        className={`text-sm p-2 rounded-lg ${message.sender === 'right' ? 'bg-blue-100 self-end' : 'bg-gray-100 self-start'
          }`}
      >
        {message.text}
      </div>
